...
function acquireLock(workspace: string, opts: InstallOptions): void {
  ...
  if (existing) {
    ...
    // `staleMs: 0` means "any existing lock counts as stale". Some
    // filesystems can report a just-written mtime fractionally in the future,
    // so special-case non-positive thresholds instead of relying on age >= 0.
    const stale = staleMs <= 0 || age >= staleMs;
