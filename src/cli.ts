...
async function handleCliOnly(command: string, args: string[]) {
  ...
  if (command === 'frontmatter-apply') {
    const { runFrontmatterApply } = await import('./commands/frontmatter-apply.ts');
    await runFrontmatterApply(args);
    return;
  }
  ...
  if (command === 'source-health') {
    const { runSourceHealth } = await import('./commands/source-health.ts');
  ...
  if (command === 'capability-catalog') {
    const { runCapabilityCatalogCommand } = await import('./commands/capability-catalog.ts');
  ...
  if (command === 'promotion-queue') {
    const { runPromotionQueueCommand } = await import('./commands/promotion-queue.ts');
  ...
  if (command === 'route-suggest') {
    const { runRouteSuggestCommand } = await import('./commands/route-suggest.ts');
    await runRouteSuggestCommand(args);
    return;
  }
  if (command === 'fleet-digest') {
    const { runFleetDigestCommand } = await import('./commands/fleet-digest.ts');
  ...
  try {
    switch (command) {
      ...
      case 'orphan-report': {
        const { runOrphanReport } = await import('./commands/orphan-report.ts');
        await runOrphanReport(engine, args);
        break;
      }
      case 'retrieval-experiment': {
        const { runRetrievalExperimentCommand } = await import('./commands/retrieval-experiment.ts');
        await runRetrievalExperimentCommand(engine, args);
        break;
      }
    }
  } finally {
...
LINKS
  ...
  orphan-report [--output dir]       Report graph orphan pages; optional index artifact
  retrieval-experiment --canonical   Gated baseline-vs-candidate search experiment
...
TOOLS
  ...
  frontmatter-apply <report.json> --dry-run
                                      Generate allowlisted frontmatter patch artifact
